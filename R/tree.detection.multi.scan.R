...
tree.detection.multi.scan <- function(data, single.tree = NULL,
  ...
  #
  # buf.2 <- sf::st_as_sf(data.frame(stem.2), coords = c("x","y"))
  ...
  for(cuts in breaks){
    ...
    .error <- try(suppressMessages(dbscan::dbscan(.cut[, c("x", "y"), drop = FALSE], eps = .eps, minPts = 10)))
